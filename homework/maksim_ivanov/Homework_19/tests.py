...
def test_create_object(start_end_session, before_after_test, payload: Dict):
    new_obj_id = create_object(base_url, headers, payload)
    get_new_obj = get_one_object(base_url, headers, new_obj_id)
    del get_new_obj["id"]
...
def test_put_object(before_after_test, new_object_id):
    put_obj = put_object(base_url, headers=headers, obj_id=new_object_id, payload=payload_put)
    del put_obj["id"]
...
def test_patch_object(before_after_test, new_object_id):
    patch_obj = patch_object(base_url, headers=headers, obj_id=new_object_id, payload=payload_patch)
    del patch_obj["id"]
